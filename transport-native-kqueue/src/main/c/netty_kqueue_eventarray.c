...
static void netty_kqueue_eventarray_evSet(JNIEnv* env, jclass clzz, jobject keventAddress, jint ident, jshort filter, jshort flags, jint fflags) {
    EV_SET((struct kevent *)(*env)->GetMemoryAddress(env, keventAddress), ident, filter, flags, fflags, 0, NULL);
}
